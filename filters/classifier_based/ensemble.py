...
class EnsembleFiltering(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for est_idx, est in enumerate(self.estimators):
            for train_indices, test_indices in skf.split(X, y_idx):
                ...
                if hasattr(model, "predict_proba"):
                    proba = np.asarray(model.predict_proba(X[test_indices]), dtype=float)
                    if proba.ndim == 2 and proba.shape[0] == test_indices.shape[0] and proba.shape[1] == self.classes_.shape[0]:
                        oof_confidence[est_idx, test_indices] = np.take_along_axis(proba, pred_idx[:, None], axis=1).ravel() # Confianza del modelo EN SU PROPIA PREDICCIÓN
        ...
        for est_idx in range(m):
            np.add.at(support, (sample_range, oof_preds[est_idx]), oof_confidence[est_idx])
