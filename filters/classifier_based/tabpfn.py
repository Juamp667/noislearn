"""
Single-classifier label-noise filtering using TabPFN.
"""
...
from .classification import ClassificationFilterResult


def _make_tabpfn_classifier(**kwargs):
    try:
        from tabpfn import TabPFNClassifier
    except ModuleNotFoundError as e:
        raise ImportError("tabpfn is required for TABPFNClassificationFilter.") from e

    return TabPFNClassifier(**kwargs)


class TABPFNClassificationFilter(BaseEstimator):
    def __init__(self, cv=10, action="remove", random_state=33, tabpfn_params=None):
        self.cv = cv
        self.action = action
    ...
    def _build_estimator(self):
        params = {} if self.tabpfn_params is None else dict(self.tabpfn_params)
        params.setdefault("random_state", self.random_state)
        params.setdefault("device", "auto")
        return _make_tabpfn_classifier(**params)
    ...
    def get_filter_report(self):
        return {
            ...
            "cv": int(self.cv),
            "action": self.action,
            "tabpfn_params": {} if self.tabpfn_params is None else dict(self.tabpfn_params),
        }
