"""
Legacy Iterative Noise Filter based on the Fusion of Classifiers.

This module keeps the previous OOF-based variant under a legacy name so the
new filter implementation can be added separately.
"""
...
@dataclass
class INFFC_old_wrongIterationInfo:
...
@dataclass
class INFFC_old_wrongFilterResult:
    ...
    n_models: int
    n_iters: int
    history: List[INFFC_old_wrongIterationInfo]


class INFFC_old_wrong(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        m = len(estimators)
        if m < 2:
            raise ValueError("Provide at least 2 estimators for INFFC_old_wrong.")
        ...
        for it in range(1, int(self.max_iter) + 1):
            ...
            for est_idx, est in enumerate(estimators):
                for train_idx, test_idx in skf.split(X_E, y_E):
                    ...
                    # Compute and store oof preds associated to the `est` estimator
            ...
            history.append(INFFC_old_wrongIterationInfo(it, nE, n_flagged, frac_flagged, self.decision_rule))
        ...
        self.result_ = INFFC_old_wrongFilterResult(
            keep_mask=keep_mask,
        ...
        self.X_ = X
        self.y_ = y_out
        # noise_score is not implemented yet for INFFC_old_wrong.
