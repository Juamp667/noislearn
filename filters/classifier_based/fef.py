...
class FilterEnsembleFilter(BaseEstimator):
    ...
    refit_base_filters : bool, default=True
        Whether to clone and fit the base filters inside :meth:`fit`. When
        False, the provided filters are treated as already fitted.
    ...
    def __init__(
        ...
        self.refit_base_filters = refit_base_filters
    ...
    def _validate_parameters(self, n_filters: int) -> None:
        ...
        if not isinstance(self.refit_base_filters, bool):
            raise TypeError("refit_base_filters must be a boolean.")
    ...
    def _fit_base_filter(self, name: str, estimator, X, y, n_samples: int):
        ...
        return self._collect_base_filter_outputs(name, fitted, n_samples, y)

    def _collect_base_filter_outputs(self, name: str, fitted_filter, n_samples: int, y):
        report = _extract_report(fitted_filter)
        noisy_mask = _extract_noisy_mask(report, fitted_filter, n_samples)
        if noisy_mask.shape[0] != n_samples:
            raise ValueError(f"Base filter '{name}' noisy_mask length does not match n_samples={n_samples}.")

        noise_score = _extract_noise_score(report, fitted_filter)
        ...
        return fitted_filter, noisy_mask, noise_score, report

    def _use_prefitted_base_filter(self, name: str, fitted_filter, n_samples: int, y):
        return self._collect_base_filter_outputs(name, fitted_filter, n_samples, y)
    ...
    def fit(self, X, y):
        ...
        for filter_idx, (name, estimator) in enumerate(resolved):
            if self.refit_base_filters:
                _, noisy_mask, noise_score, report = self._fit_base_filter(name, estimator, X, y, n_samples)
            else:
                _, noisy_mask, noise_score, report = self._use_prefitted_base_filter(name, estimator, n_samples, y)
